...
export class AnimationManager {
    constructor(containerId, config = {}) {
        ...
        this.config = {
            ...
            duration: config.type === 'landing' ? 120 : 16, // 120 seconds for landing, 16 for header
            ...config
        };
    ...
    updateLandingColors() {
        ...
        const normalizedTime = (currentTime % this.config.duration) / this.config.duration;
        const pulseIntensity = (Math.sin(normalizedTime * Math.PI * 2) + 1) * 0.5;
    ...
    animateLanding(deltaTime) {
        ...
        if (this.wireframe) {
            this.wireframe.rotation.x = Math.sin(angle) * Math.PI * 0.25;
            this.wireframe.rotation.y = Math.cos(angle) * Math.PI * 0.25;
            this.wireframe.rotation.z = Math.sin(angle * 0.5) * Math.PI * 0.125;
        ...
        if (this.sphere) {
            this.sphere.rotation.x = -Math.sin(angle * 0.8) * Math.PI * 0.25;
            this.sphere.rotation.y = -Math.cos(angle * 0.8) * Math.PI * 0.25;
        }
    }
    ...
    animateHeader(deltaTime) {
        ...
        this.planes.forEach((plane, i) => {
            ...
            plane.rotation.z += speed * deltaTime;
            plane.rotation.x = Math.sin(normalizedTime * Math.PI * 2 * speed) * 0.1;
            plane.rotation.y = Math.cos(normalizedTime * Math.PI * 2 * speed) * 0.1;
        });
    }
